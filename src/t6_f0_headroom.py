...
def main():
    ...
    num["residual_test_reading"] = {
        ...
        "answer": "One comparison of fourteen survives Bonferroni correction, "
                  ...
                  "exceedance elsewhere points the opposite way. A named "
                  "alternative explains it equally well: any factor separating "
                  "the control from the ladder, such as prompt formatting, option "
                  "tokenization, or a different prior over option positions, "
                  "produces a choice-rate shift of this size, and this design "
                  "cannot distinguish those from an adversary residual. So: not nothing, "
...
def write_report(num, secs):
    ...
    if not mc["survivors"]:
        ...
        w(f"1. **The effect is about {round(c['excess_beyond_band'] * n_rend)} "
          ...
          f"clear it.\n")
        w(f"2. **It is on `CTRL` alone, and a named alternative explains it as "
          f"well as an adversary residual does.** No ladder model survives. "
          f"`CTRL` is the cross-family control, differing from the ladder in "
          f"model family and in tokenizer, and `share_negative` is a choice-based "
          f"rate, which is the class P1's D111 permits for exactly that reason. "
          f"**Any factor that separates the control from the ladder would produce "
          f"a choice-rate shift of about this size: prompt formatting, "
          f"tokenization of the option strings, or a different prior over option "
          f"positions.** Nothing in this design distinguishes those from an "
          f"adversary-structure residual on a single model, and no measurement "
          f"here can. This is stated as the competing explanation it is, not as a "
          f"reason to discount the finding, which stays in the table either "
          f"way.\n")
